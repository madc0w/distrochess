// taken from https://github.com/iDoRecall/email-normalize/blob/master/email.js
// because npm install somehow fails, and there is no guidance on how to install this
...
Email.normalize = function normalizeEmail(email, options, callback) {
	...
	if (options.forceRemoveTags) {
		...
		const separator = Email.domainsWithTags[domain];
		if (separator) {
	...
	if (options.detectProvider) {
		...
		if (Meteor.isClient) {
			...
			return HTTP.get("http://enclout.com/api/v1/dns/show.json", {
				params : {
					url : domain
				}
				...
				for (var i = 0; i < addresses.length; i++) {
					if (!addresses[i].Type) {
						return callback("Could not find Type field in Enclout API result. Has the format changed?");
					}
					if (addresses[i].Type === "MX") {
						user = processMXRecords(addresses[i].RData, user);
					}
				}
			...
			const resolveMx = Meteor.wrapAsync(Npm.require("dns").resolveMx);
			if (typeof callback === "function") {
				...
				const addresses = resolveMx(domain);
				for (var i = 0; i < addresses.length; i++) {
					user = processMXRecords(addresses[i].exchange, user);
				}
				return user + "@" + domain;
